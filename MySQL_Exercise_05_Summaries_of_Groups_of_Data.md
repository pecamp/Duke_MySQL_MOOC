...
```python
%%sql
SELECT breed_group, gender, COUNT(*) AS num_dogs
FROM dogs
...
SELECT breed_group, gender, COUNT(*) AS num_dogs
FROM dogs
WHERE breed_group IS NOT NULL OR breed_group != ""
GROUP BY 1, 2
HAVING COUNT(num_dogs) >= 1000
ORDER BY num_dogs DESC;
```
...
```python
%%sql
SELECT breed_type, AVG(total_tests_completed) AS Avg_Num_Tests, AVG(mean_iti_minutes)
FROM dogs
GROUP BY breed_type
ORDER BY Avg_Num_Tests DESC;
```
